...
public class ListenerAbrirEnv {
	...
	public static String clicouBotaoGerar(byte[] msgCript, byte[] chaveCript,
		...
		// Verifica se o arquivo de mensagem criptografada esta vazio ou se o usuário não o selecionou
		if (msgCript == null || msgCript.length == 0)
		{
			return "1 - Arquivo de texto vazio/não selecionado.";
		}

		// Verifica se o arquivo de chave criptografada esta vazio ou se o usuário não o selecionou
		else
		if (chaveCript == null || chaveCript.length == 0)
		{
			return "1 - Arquivo de chave simétrica criptografada vazio/não selecionado.";
		}

		// Verifica se o arquivo da chave privada RSA esta vazio ou se o usuário não o selecionou
		else
		...
		// Verifica se o arquivo da chave privada RSA utiliza o padrão de chaves geradas pela nossa aplicação (header e footer)
		else
